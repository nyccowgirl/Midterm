...
/**
 A class that implements a list of objects by using an array. Entries in a * list have positions that begin with 1.
 Duplicate entries are allowed. * * @author Frank M. Carrano * @author Timothy M. Henry * @version 5.0
 */
...
//		}
		...
		int capacity = list.length - 1;
		if ((numberOfEntries + count) >= capacity) {
			int newCapacity = (numberOfEntries + count) + 1;			// Ensures enough capacity in case chain is bigger than capacity of current list
			checkCapacity(newCapacity); // Is capacity too big?
			list = Arrays.copyOf(list, newCapacity + 1);		// O(n)
		}
